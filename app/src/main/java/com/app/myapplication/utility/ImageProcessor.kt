package com.app.myapplication.utility
...
object ImageProcessor {
    ...
    /*fun loadProfilePic(imageView: ImageView, imageUrl: String = "") =
        Glide.with(imageView.context).load(
            if (imageUrl.contains("http")) imageUrl else ApiConstants.USER_IMAGE_LOADER.plus(imageUrl)
        ).transition(GenericTransitionOptions.with(R.anim.fade_in))
            .apply(AppConstants.requestOptions!!).into(imageView)

    fun loadResizedProfilePic(imageView: ImageView, imageUrl: String = "", size: Int) =
        Glide.with(imageView.context).load(
            if (imageUrl.contains("http")) imageUrl else ApiConstants.USER_IMAGE_LOADER.plus(imageUrl)
        ).transition(GenericTransitionOptions.with(R.anim.fade_in))
            .apply(AppConstants.requestOptions!!.override(size, size))
            .into(imageView)

    fun loadAdvertisement(advertisementUrl: String, imageView: ImageView) =
        Glide.with(imageView.context).load(ApiConstants.ADVERTISEMENT_LOADER.plus(advertisementUrl))
            .thumbnail(0.1F).transition(GenericTransitionOptions.with(R.anim.fade_in))
            .apply(AppConstants.requestOptions!!).into(imageView)

    fun loadThumbnailFromUrl(imageUrl: String, imageView: ImageView) =
        Glide.with(imageView.context).load(ApiConstants.CHAT_IMAGE_LOADER.plus(imageUrl))
            .thumbnail(0.1F).transition(GenericTransitionOptions.with(R.anim.fade_in))
            .apply(AppConstants.requestOptions!!).into(imageView)
    ...
    fun loadThumbnailFromFile(imagePath: String, imageView: ImageView) {
        Glide.with(imageView.context)
            ...
            .transition(GenericTransitionOptions.with(R.anim.fade_in))
            .apply(AppConstants.requestOptions!!)
            .into(imageView)
    }
    ...
    fun compressImage(imagePath: String, destinationPath: String = FileIO.getTempFilename()): String {
        ...
        try {
            ...
            Logs.printMessages(TAG, exception.message)
        }
